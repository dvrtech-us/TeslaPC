...
- While loopback has been idle for at least `SilenceKeepaliveStartMs` (250 ms) but less than `SilenceKeepaliveMaxSeconds` (30 s), and the broadcast queue is empty, the server injects zero-filled PCM chunks sized to the last `DataAvailable` buffer so clients keep decoding through short show-silence gaps. Shorter gaps are normal between WASAPI buffers during active playback and must not trigger keepalive.
